...
class WorkWithArrays {
    ...
    /* Utility che restituisce un array con i soli elementi di indice pari */
    static int[] evenElements(final int[] array) {
        final int[] returnValue = new int[(array.length + 1) / 2];
    ...
    /* Utility che restituisce un array con i soli elementi di indice dispari */
    static int[] oddElements(final int[] array) {
        final int[] returnValue = new int[array.length / 2];
    ...
    /* Utility che restituisce l'elemento che appare più frequentemente nell'array */
    static int mostRecurringElement(final int[] array) {
        int maxCount = 0;
        int mostRecurring = array[0];
        for (int i = 0; i < array.length; i++) {
            int count = countOccurrencies(array, array[i]);
            if (count > maxCount) {
                maxCount = count;
                mostRecurring = array[i];
            }
        }
    ...
    static int[] sortArray(final int[] array, final boolean isDescending) {
        final int[] returnValue = Arrays.copyOf(array, array.length);
        Arrays.sort(returnValue);
        if (isDescending) {
            for (int i = 0; i < returnValue.length / 2; i++) {
                int temp = returnValue[i];
                returnValue[i] = returnValue[returnValue.length - 1 - i];
                returnValue[returnValue.length - 1 - i] = temp;
            }
        }
        return returnValue;
    }
    ...
    static int[] revertUpTo(final int[] array, final int element) {
        int[] returnValue = Arrays.copyOf(array, array.length);
        int i = 0;
        while (i < returnValue.length && returnValue[i] != element) {
            i++;
        }
        if (i < returnValue.length) {
            // Invert the elements from the beginning up to and including the first occurrence of 'element'
            for (int j = 0; j <= i; j++) {
                returnValue[j] = array[i - j];
            }
        }
        return returnValue;
    }
    ...
    static boolean testCountOccurrencies() {
        return countOccurrencies(new int[] { 1, 2, 3, 4 }, 1) == 1
            ...
            && countOccurrencies(new int[] { 1, 2, 3, 4, 5, 6, 7, 8, 9 }, 5) == 1;
    }
